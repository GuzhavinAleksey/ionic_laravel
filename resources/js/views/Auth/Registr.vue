...
const submit = () => {
  try {
    ...
    Connect.post('/auth/registr', datavw.value.form)
      .then((response) => {
        response.email = '';
        response.password = '';
        response.password_confirmation = '';
...
<IonInput id="password" name="password" v-model="datavw.form.password_confirmation" type="password" autocomplete="current-password" required="" class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-b-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Повторите пароль"></IonInput>
        ...
        <div>
          ...
          <IonButton
            @click="() => router.navigate('/auth/login','forward','replace')"

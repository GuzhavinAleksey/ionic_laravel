...
const sessioncheck = () => {
    if(!token) {
        router.navigate('/auth/login','forward','replace');
    }
};
...
<template>
    <div class="sticky top-0 z-40">
            <div class="w-full h-20 px-6 bg-gray-100 border-b flex items-center justify-between">
              ...
              <div class="flex">
                ...
                <div class="inline-block lg:hidden flex items-center mr-4">
                  <IonButton class="hover:text-blue-500 hover:border-white focus:outline-none navbar-burger" @click="toggleSidebar()">
                    <svg class="h-5 w-5" v-bind:style="{ fill: 'black' }" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><title>Меню</title><path d="M0 3h20v2H0V3zm0 6h20v2H0V9zm0 6h20v2H0v-2z"/></svg>
                  </IonButton>
                </div>
                ...
                <!--<div class="relative text-gray-600">
                  ...
                  <IonButton type="submit" class="absolute right-0 top-0 mt-3 mr-4">
          ...
          <IonText>
                <h5 @click="dropDownOpen = !dropDownOpen"> {{user.email}} </h5>
          </IonText>
              </div>
            ...
            <div class="absolute bg-gray-100 border border-t-0 shadow-xl text-gray-700 rounded-b-lg w-48 right-0 mr-6" :class="dropDownOpen ? '' : 'hidden'">
                <IonButton class="block px-4 py-2 hover:bg-gray-200" @click="logout()">Выход</IonButton>
            </div>
            <!-- dropdown menu end -->
    </div>
</template>
